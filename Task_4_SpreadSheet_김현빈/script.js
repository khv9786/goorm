...
// AA, AZ 구현
function generateAlphabetPairs(alphabets) {
  let result = [];
  ...
  for (let i = 0; i < alphabets.length; i++) {
    for (let j = 0; j < alphabets.length; j++) {
      result.push(alphabets[i] + alphabets[j]);
    }
  }
  return result;
}
...
const alphabetPairs = generateAlphabetPairs(alphabets);
...
function initSpreadsheet() {
  for (let i = 0; i < ROWS; i++) {
    ...
    for (let j = 0; j < COLS; j++) {
      ...
      const rowName = i;
      const columnName = alphabetPairs[j - 1];
